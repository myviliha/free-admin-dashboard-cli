...
/**
 * Which package manager launched us, when that is a real choice.
 *
 * yarn, pnpm and bun each set `npm_config_user_agent` to their own name, and reaching for one of
 * those is a decision: `pnpm create ...` or `bunx ...` says which tool the project should use.
 *
 * **npm is the exception, and it is deliberately ignored.** `npx` sets the same `npm/...` user agent
 * as `npm create`, and `npx` is how nearly everyone runs this. Treating that as a request for npm
 * would make pnpm the documented default and npm the actual one for most people. `--npm` is there
 * for anyone who wants it.
 *
 * Order matters regardless: pnpm's user agent mentions npm as well, so a naive `includes("npm")`
 * reports npm for every pnpm run.
 */
...
/**
 * pnpm if it is installed, npm if it is not.
 *
 * Defaulting to a tool the user does not have would turn a working scaffold into a failed install at
 * the last step, having already downloaded everything.
 */
export function defaultPM(has = (cmd) => {
  try {
...
export function parseArgs(argv, userAgent = process.env.npm_config_user_agent) {
  ...
  // An explicit flag wins, then an unambiguous launcher, then pnpm if it is installed.
  opts.pm = opts.pm ?? detectPM(userAgent) ?? defaultPM();
  return opts;
}
